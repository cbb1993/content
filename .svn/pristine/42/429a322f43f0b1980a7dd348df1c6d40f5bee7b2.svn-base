package com.huanhong.content.model.api.netty;
            ...
            switch (msg.what)
            {
                case MESSAGE_INIT:
                {
                    ...
                    mBootstrap.handler(new ChannelInitializer<SocketChannel>()
                    {
                        ...
                        protected void initChannel(SocketChannel ch) throws Exception
                        {
                            ...
                            pipeline.addLast("framer", new DelimiterBasedFrameDecoder(1024 * 2 * 1024, Delimiters.lineDelimiter()));
                            pipeline.addLast("decoder", new StringDecoder(CharsetUtil.UTF_8));
                            pipeline.addLast("encoder", new StringEncoder(CharsetUtil.UTF_8));
                            pipeline.addLast("ping", new IdleStateHandler(15, 10, 10, TimeUnit.SECONDS));
                ...
                case MESSAGE_CONNECT:
                {
                    try
                    {
                        ...
                        {
                            mChannel = mBootstrap
                                    .connect(new InetSocketAddress(mHost,
                                            mPort))
                                    .sync()
                                    .channel();
                        }
                ...
                case MESSAGE_SEND:
                {
                    ...
                    try
                    {
                        if (mChannel != null && mChannel.isActive())
                        {
                            ...
                            mChannel
                                    .writeAndFlush(constructMessage(sendMsg))
                                    .sync();
                            ...
                            mMainHandler.post(new Runnable()
                            {
                                ...
                                public void run()
                                {
                                    if (mNettyClientListener != null)
                                    {
                                        mNettyClientListener.onSent(constructMessage(sendMsg));
                                    }
                                }
                            });
                        } else
